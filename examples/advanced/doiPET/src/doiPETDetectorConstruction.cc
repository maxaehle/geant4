  ...
  fCheckOverlaps(true)
{
  ...
  PhantomType = "NEMA_imageQualityPhantom_wholeBody";//"Phantom_sensitivity";// "NEMA_phantom_NECR";//  
...
void doiPETDetectorConstruction::ConstructPhantom(G4LogicalVolume* worldLogical)
{
  ...
  if(PhantomType == "NEMA_imageQualityPhantom_wholeBody"){
    ...
    //The size of the phantom can be changed via the macro file by changing the number of sleeves
    //The length of sensitivity phantom is 700 mm
    //(3.9, 6.4), (7.0, 9.5), (10.2, 12.7), (13.4, 15.9), (16.6, 19.1)

    G4double Rmin, Rmax;
    G4cout<<"Total number of sleeves : "<<numOfSleeves<<G4endl;

    for(G4int i=0; i<numOfSleeves; i++){
      if(i==0){
        Rmin = 3.9/2 * mm;
        Rmax = 3.2 * mm; 
        }
      ...
      G4cout<<"Sleeve "<<i+1 <<" is placed, Rmin = "<<Rmin<< " mm, Rmax = "<<Rmax<< " mm, "<< "length = " <<phantomLength <<" mm."<<G4endl;

      //Concenric aluminum sleeves
      G4Tubs* phantom = new G4Tubs("Phantom_sensitivity", Rmin,Rmax,phantomLength/2,0*deg,360*deg);
    ...
    //Define its physical volume
    phantomPE_physicalV = new G4PVPlacement(0, phantomPosition, phantomPE_logicalV, "phantom_physicalV", worldLogical, false, 0,fCheckOverlaps);
...
//
void doiPETDetectorConstruction::SetNumberOfSleeves(G4int NewNumOfSleeves){
  numOfSleeves = NewNumOfSleeves;
  //G4cout<<"numOfSleeves "<<numOfSleeves<<G4endl;
}
