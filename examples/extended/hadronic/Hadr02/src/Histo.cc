...
void Histo::Book()
{
  ...
  if ( fHistType == "root" ) {
    ...
    fManager = G4XmlAnalysisManager::Instance();
  }
...
void Histo::Save()
{
  ...
  if(!fManager->Write()) {
    ...
    message << "Histo::Save: FATAL ERROR writing ROOT file."; 
    G4Exception("Histo::Save()",
                "Hadr02Code001", FatalException, message);
  }
  if(fVerbose > 0) {
...
void Histo::Fill(G4int i, G4double x, G4double w)
{
  ...
  if(i>=0 && i<fNHisto) {
    ...
    G4cout << "Histo::Fill: WARNING! wrong histogram index " 
          << i << G4endl;
  }
}
...
void Histo::ScaleH1(G4int i, G4double x)
{
  ...
  if(i>=0 && i<fNHisto) {
    if(fActive[i]) { 
      if ( fHistType == "root" )
   dynamic_cast<G4RootAnalysisManager*>(fManager)->GetH1(fHisto[i])->scale(x); 
      if ( fHistType == "xml" )
      dynamic_cast<G4XmlAnalysisManager*>(fManager)->GetH1(fHisto[i])->scale(x);
    }
  } else {
