...
{
  fHelper = std::make_unique<G4AnalysisMessengerHelper>("plot");
...
{
  fSetStyleCmd = std::make_unique<G4UIcmdWithAString>("/analysis/plot/setStyle",this);
...
void G4PlotMessenger::SetLayoutCmd()
{
  ...
  auto maxColumns = fPlotParameters->GetMaxColumns();
  range.append(to_string(maxColumns));
  ...
  range = "rows>=1 && rows<=";
  auto maxRows = fPlotParameters->GetMaxRows();
  range.append(to_string(maxRows));
  rows->SetParameterRange(range);

  fSetLayoutCmd = std::make_unique<G4UIcommand>("/analysis/plot/setLayout", this);
  // Guidance text:
  ...
  guidance.append(to_string(maxColumns));
  fSetLayoutCmd->SetGuidance(guidance);
  guidance = "  rows    = 1 .. ";
  guidance.append(to_string(maxRows));
...
void G4PlotMessenger::SetDimensionsCmd()
{
  ...
  fSetDimensionsCmd = std::make_unique<G4UIcommand>("/analysis/plot/setDimensions", this);
