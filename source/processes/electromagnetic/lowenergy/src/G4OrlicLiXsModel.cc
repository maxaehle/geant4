...

{
  ...
  if ( zTarget < 41 )//fixed: no control on z!
    {
      return 0;
    }
  ...
  G4double analyticalFunction = a0 + (a1*x)+(a2*x*x)+(a3*std::pow(x,3))+
...

{
  ...
  G4double analyticalFunction = a0 + (a1*x)+(a2*x*x)+
    (a3*std::pow(x,3))+(a4*std::pow(x,4))+(a5*std::pow(x,5));
