...
G4ChipsHyperonElasticXS::G4ChipsHyperonElasticXS():G4VCrossSectionDataSet(Default_Name()), nPoints(128), nLast(nPoints-1)
{
...
G4double G4ChipsHyperonElasticXS::GetIsoCrossSection(const G4DynamicParticle* Pt, G4int tgZ, G4int A,  
							 const G4Isotope*,
							 const G4Element*,
							 const G4Material*)
{
...
G4double G4ChipsHyperonElasticXS::GetPTables(G4double LP, G4double ILP, G4int PDG,
                                                  G4int tgZ, G4int tgN)
{
  ...
  if(PDG>3000 && PDG<3335)
  {
    ...
    if(LP>ILP)
    {
      ...
      if(ini<nPoints)
      {
        ...
        else G4cout<<"*Warning*G4ChipsHyperonElasticXS::GetPTables: PDG="<<PDG
    ...
    // G4cout<<"*Error*G4ChipsHyperonElasticXS::GetPTables: PDG="<<PDG<<", Z="<<tgZ
    //       <<", N="<<tgN<<", while it is defined only for Hyperons"<<G4endl;
    // throw G4QException("G4ChipsHyperonElasticXS::GetPTables:onlyaBA implemented");
    G4ExceptionDescription ed;
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for Hyperons" << G4endl;
    G4Exception("G4ChipsHyperonElasticXS::GetPTables()", "HAD_CHPS_0000",
                FatalException, ed);
  }
  return ILP;
}
...
G4double G4ChipsHyperonElasticXS::GetExchangeT(G4int tgZ, G4int tgN, G4int PDG)
{
...
G4double G4ChipsHyperonElasticXS::GetSlope(G4int tgZ, G4int tgN, G4int PDG)
{
  static const G4double GeVSQ=gigaelectronvolt*gigaelectronvolt;
  ...
  if(PDG<3000 || PDG>3334)
  {
    ...
    //       <<", N="<<tgN<<", while it is defined only for Hyperons"<<G4endl;
    // throw G4QException("G4ChipsHyperonElasticXS::GetSlope: HypA are implemented");
    G4ExceptionDescription ed;
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for Hyperons" << G4endl;
    G4Exception("G4ChipsHyperonElasticXS::GetSlope()", "HAD_CHPS_0000",
                FatalException, ed);
  }
  if(theB1<0.) theB1=0.;
...
G4double G4ChipsHyperonElasticXS::GetQ2max(G4int PDG, G4int tgZ, G4int tgN,
                                                    G4double pP)
{
  static const G4double mLamb= G4Lambda::Lambda()->GetPDGMass()*.001; // MeV to GeV
  ...
  if(tgZ || tgN>-1)                                   // --> Hyperon-A
  {
    ...
    G4double dmt=mt+mt;
    G4double mds=dmt*std::sqrt(pP2+mLa2)+mLa2+mt*mt;    // Mondelstam mds (@@ other hyperons?)
    return dmt*dmt*pP2/mds;
  }
  else
  {
    // G4cout<<"*Error*G4ChipsHyperonElasticXS::GetQ2ma:PDG="<<PDG<<",Z="<<tgZ<<",N="
    //       <<tgN<<", while it is defined only for p projectiles & Z_target>0"<<G4endl;
    // throw G4QException("G4ChipsHyperonElasticXS::GetQ2max: only HyperA implemented");
    G4ExceptionDescription ed;
    ed << "PDG = " << PDG << ", Z = " << tgZ << ", N = " << tgN
       << ", while it is defined only for p projectiles & Z_target>0" << G4endl;
    G4Exception("G4ChipsHyperonElasticXS::GetQ2max()", "HAD_CHPS_0000",
                FatalException, ed);
    return 0;
  }
}
