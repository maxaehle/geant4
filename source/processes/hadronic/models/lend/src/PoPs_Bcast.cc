...
typedef struct PoPs_Bcast_info {
    ...
    int int_count, char_count, G4double_count;
    int *int_array;
    char *char_array;
    G4double *G4double_array;
} PoPs_Bcast_info;
...
static int PoPs_Bcast_G4double( statusMessageReporting *smr, PoPs_Bcast_info *info, G4double *value );
/*
========================================================================
*/
...
*/
    ...
    if( myRank == bossRank ) {
        ...
        description[2] = info.G4double_count;
        ...
        if( ( info.G4double_array = (G4double *) smr_malloc2( smr, info.G4double_count * sizeof( G4double ), 1, "info.G4double_array" ) ) == NULL ) goto err;

        info.mode = PoPs_Bcast_mode_pack;
        info.int_count = 0;
        info.char_count = 0;
        info.G4double_count = 0;
        if( ( status = PoPs_Bcast3( smr, comm, &info, unitsRoot, popsRoot ) ) != 0 ) return( status );
        ...
        if( info.G4double_count != description[2] ) {
            smr_setReportError2( smr, PoPs_smr_ID, 1, "G4double counting count = %d != packing count = %d", info.G4double_count, description[2] );
            goto err;
        }
    }
    ...
    if( myRank != bossRank ) {
        ...
        if( ( info.G4double_array = (G4double *) smr_malloc2( smr, description[2] * sizeof( G4double ), 1, "info.G4double_array (2)" ) ) == NULL ) goto err;
    }
    ...
    if( ( status = MPI_Bcast( info.G4double_array, description[2], MPI_DOUBLE, bossRank, comm ) ) != 0 ) goto err;
    ...
    if( info.G4double_array != NULL ) smr_freeMemory( (void **) &(info.G4double_array) );
...
static int PoPs_Bcast_PoPs2( statusMessageReporting *smr, PoPs_Bcast_info *info, PoP *pop ) {
    ...
    if( ( status = PoPs_Bcast_G4double( smr, info, &(pop->mass) ) ) != 0 ) return( status );
...
static int PoPs_Bcast_G4double( statusMessageReporting *smr, PoPs_Bcast_info *info, G4double *value ) {
    ...
    if( info->mode == PoPs_Bcast_mode_pack ) {
        info->G4double_array[info->G4double_count] = *value; }
    else if( info->mode == PoPs_Bcast_mode_unpack ) {
        *value = info->G4double_array[info->G4double_count];
    }
    info->G4double_count++;
    return( 0 );
}
