...
void NonRandomEngine::setNextRandom(G4double r) {
  nextRandom = r;
  nextHasBeenSet=true;
  return;
}

void NonRandomEngine::setRandomSequence(G4double* s, int n) {
  sequence.clear();
...
void NonRandomEngine::setRandomInterval(G4double x) {
  randomInterval = x;
  intervalHasBeenSet=true;
  return;
}

G4double NonRandomEngine::flat() {

  if (sequenceHasBeenSet) {
    G4double v = sequence[nInSeq++];
...
void NonRandomEngine::flatArray(const int size, G4double* vect) {
...
bool NonRandomEngine::getState (const std::vector<unsigned long> & v) {
  ...
  t[0] = v[4]; t[1] = v[5]; nextRandom = DoubConv::longs2G4double(t);
  nInSeq = v[6];
  t[0] = v[7]; t[1] = v[8]; randomInterval = DoubConv::longs2G4double(t);
  sequence.clear();
  for (unsigned int i=0; i<seqSize; ++i) {
    t[0] = v[2*i+10]; t[1] = v[2*i+11];
    sequence.push_back(DoubConv::longs2G4double(t));
  }
  return true;
}
