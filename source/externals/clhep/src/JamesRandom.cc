...
void HepJamesRandom::flatArray(const int size, G4double* vect)
{
//   G4double uni;
   int i;
...
HepJamesRandom::operator G4double() {
   return flat();
}

HepJamesRandom::operator G4float() {
   return G4float( flat() );
}
...
bool HepJamesRandom::getState (const std::vector<unsigned long> & v) {
  ...
  for (int i=0; i<97; ++i) {
    ...
    u[i] = DoubConv::longs2G4double(t);
  }
  t[0] = v[195]; t[1] = v[196]; c  = DoubConv::longs2G4double(t);
  t[0] = v[197]; t[1] = v[198]; cd = DoubConv::longs2G4double(t);
  t[0] = v[199]; t[1] = v[200]; cm = DoubConv::longs2G4double(t);
  j97  = v[201];
  i97  = (64+j97)%97; 
  return true;
}
