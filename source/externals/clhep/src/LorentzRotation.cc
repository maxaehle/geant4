...
//      << std::endl;
//  }    
  G4double gamma = 1.0 / std::sqrt(1.0 - bp2);
  G4double bgamma = gamma * gamma / (1.0 + gamma);
...
G4double HepLorentzRotation::operator () (int i, int j) const {
  if (i == 0) {
    if (j == 0) { return xx(); }
    if (j == 1) { return xy(); }
    if (j == 2) { return xz(); } 
...
HepLorentzRotation & HepLorentzRotation::rotateX(G4double delta) {
  G4double c1 = std::cos (delta);
  G4double s1 = std::sin (delta);
...
HepLorentzRotation & HepLorentzRotation::rotateY(G4double delta) {
  G4double c1 = std::cos (delta);
  G4double s1 = std::sin (delta);
...
HepLorentzRotation & HepLorentzRotation::rotateZ(G4double delta) {
  G4double c1 = std::cos (delta);
  G4double s1 = std::sin (delta);
  HepLorentzVector rowx = row1();
  HepLorentzVector rowy = row2();
...
HepLorentzRotation & HepLorentzRotation::boostX(G4double beta) {
  G4double b2 = beta*beta;
//  if (b2 >= 1) {
//    std::cerr << "HepLorentzRotation::boostX() - "
//      << "Beta supplied to HepLorentzRotation::boostX represents speed >= c."
//      << std::endl;
//  }    
  G4double g1  = 1.0/std::sqrt(1.0-b2);
  G4double bg = beta*g1;
  HepLorentzVector rowx = row1();
...
HepLorentzRotation & HepLorentzRotation::boostY(G4double beta) {
  G4double b2 = beta*beta;
//  if (b2 >= 1) {
...
HepLorentzRotation & HepLorentzRotation::boostZ(G4double beta) {
  G4double b2 = beta*beta;
//  if (b2 >= 1) {
